...
- PCB 标注悬浮层统一（`3.49`）：整体模型缩放调整为 `1.1x`，相机取景宽度调整为 `27` 为旋转预留边界；白色虚线框、引导线、端点和文字统一到各自悬浮高度，并关闭深度测试避免层级错位。
- PCB 正面标注层修复（`3.50`）：引导线和提示文字移出动态旋转层并固定在前方，PCB 转到背面时仅隐藏正面白色虚线框；按 `1.1x` 缩放重新计算五组连接点，标题顶部间距增加到 `50px`。
...
- PCB 标注深度修复（`3.56`）：恢复引导线、端点和提示文字的默认深度测试，避免它们穿透 PCB 背面底板显示。
- PCB 热点标注悬浮联动（`3.57`）：为每个热点建立独立虚线框和引导线引用，悬浮时同步放大至 `1.08x`、提高透明度并启用 `AdditiveBlending`，形成高亮发光效果。
- PCB 提示层交互联动（`3.58`）：复用热点 `bind()` 事件，使提示文字和引导线悬浮时触发同样高亮效果，点击后执行与 PCB 按钮一致的页面或区块跳转。
- 个人介绍联系方式分栏（`3.59`）：联系方式和仓库地址桌面端改为两列响应式网格，移动端恢复单列，并增加长地址换行保护。
- 个人介绍信息可读性优化（`3.60`）：联系方式标签和经历统计标签/辅助说明字号略微增大并提亮颜色，增强与深色背景的对比度。
...
- 博客文章接口与数据表（`3.65`）：新增 Worker `articles` 表及排序/状态索引，新增文章 DTO 和输入校验，实现已发布文章列表/详情接口，以及管理员文章列表、新增、编辑、删除接口；`node --check` 与 `wrangler deploy --dry-run` 验证通过。
- 博客前台页面（`3.66`）：新增 `/blog` 文章列表和 `/blog/:id` 文章详情，接入文章公开接口；移除首页联系区块和导航联系按钮，将 PCB/移动端入口与站内搜索的联系入口改为博客入口；`npm run build` 验证通过。
- 首页页脚恢复（`3.69`）：从原联系区提取独立 `Footer` 组件，在首页底部恢复版权、技术方向和「回到顶部」，不恢复联系区内容。
- 后台文章管理入口（`3.70`）：新增文章管理标签页、文章列表、草稿/发布状态、排序、封面地址、Markdown 工具栏正文编辑器、保存和删除流程；`npm run build` 验证通过。
- wangEditor 文章编辑器（`3.71`）：接入 wangEditor，支持 H1-H5、字号、富文本图片和文章封面上传；新增 Worker KV 图片上传接口与基础 HTML 清洗，文章详情改为 HTML 渲染；`npm run build`、Worker 语法检查通过。
- 文章编辑预览与代码高亮（`3.72`）：编辑区改为白底黑字，新增「编辑/预览」切换，预览区使用 `highlight.js` 高亮代码块；构建验证通过。
- wangEditor 切换修复（`3.73`）：编辑器销毁时清空实例引用，避免预览切回编辑继续使用已销毁实例导致页面黑屏。
- wangEditor 黑屏二次修复（`3.74`）：编辑器改为始终挂载，仅切换显示状态，避免编辑/预览切换触发 wangEditor 卸载重建异常。
- 文章代码块展示（`3.75`）：新增统一 `ArticleHtml` 渲染组件，代码块显示语言标识、语法高亮、复制按钮和行号，不包含运行按钮。
- 代码语法配色修复（`3.76`）：加载 highlight.js `atom-one-dark` 主题，无语言标记的代码块默认按 C++ 高亮，恢复函数、循环、关键字和字符串的颜色区分。
